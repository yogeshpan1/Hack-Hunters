...
import {Audit,Rules,Analytics,Communications,Import,Exams,Settings} from "./screens/System";
...
function Content(){const {user,workspace,loading,error,refresh,logout}=useApp();if(loading)return <div className="empty">Opening your academic workspace…</div>;if(!user)return <Login/>;if(!workspace)return <div className="empty"><ErrorNotice message={error||"Loading workspace…"}/><Button onClick={()=>void refresh().catch(()=>{})}>Retry</Button><Button onClick={logout}>Sign out</Button></div>;return <><Routes><Route element={<Shell/>}><Route path="/overview" element={<Overview/>}/><Route path="/timetable" element={<Timetable/>}/><Route path="/optimization" element={<Optimization/>}/><Route path="/whatif" element={<Optimization whatif/>}/>{["rooms","faculty","programmes","modules","cohorts","students","users"].map(entity=><Route key={entity} path={`/${entity}`} element={<Records entity={entity}/>}/>)}<Route path="/audit" element={<Audit/>}/><Route path="/rules" element={<Rules/>}/><Route path="/analytics" element={<Analytics/>}/><Route path="/notifications" element={<Communications/>}/><Route path="/import" element={<Import/>}/><Route path="/exams" element={<Exams/>}/><Route path="/settings" element={<Settings/>}/><Route path="*" element={<Navigate to="/overview" replace/>}/></Route></Routes><CommandPalette/><Assistant/><Toaster/></>;}
export default function App(){return <ErrorBoundary><BrowserRouter><AppProvider><Startup/><Content/></AppProvider></BrowserRouter></ErrorBoundary>;}
