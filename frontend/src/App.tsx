...
function Content(){const location=useLocation();const {can,user,workspace,loading,error,refresh,logout}=useApp();if(loading)return <div className="empty">Opening your academic workspace…</div>;if(!user)return <Login/>;if(!workspace)return <div className="empty"><ErrorNotice message={error||"Loading workspace…"}/><Button onClick={()=>void refresh().catch(()=>{})}>Retry</Button><Button onClick={logout}>Sign out</Button></div>;const item=NAV.find(n=>`/${n.path}`===location.pathname);if(item?.roles&&!can(...item.roles))return <Navigate to="/timetable" replace/>;return <><Routes><Route element={<Shell/>}><Route path="/overview" element={<Overview/>}/><Route path="/timetable" element={<Timetable/>}/><Route path="/optimization" element={<Optimization/>}/><Route path="/whatif" element={<Optimization whatif/>}/>{["rooms","faculty","programmes","modules","cohorts","students"].map(entity=><Route key={entity} path={`/${entity}`} element={<Records entity={entity}/>}/>)}<Route path="/student-messages" element={<StudentMessages/>}/><Route path="/accounts" element={<Accounts/>}/><Route path="/audit" element={<Audit/>}/><Route path="/rules" element={<Rules/>}/><Route path="/analytics" element={<Analytics/>}/><Route path="/notifications" element={<Communications/>}/><Route path="/exams" element={<Exams/>}/><Route path="/settings" element={<Settings/>}/><Route path="*" element={<Navigate to="/overview" replace/>}/></Route></Routes><CommandPalette/><Assistant/><Toaster/></>;}
