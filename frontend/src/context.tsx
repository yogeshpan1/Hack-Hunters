...
type ContextValue={user:User|null;workspace:Workspace|null;loading:boolean;error:string;refresh:()=>Promise<void>;login:(email:string,password:string)=>Promise<void>;logout:()=>void;dark:boolean;toggleDark:()=>void;collapsed:boolean;setCollapsed:(v:boolean)=>void;palette:boolean;setPalette:(v:boolean)=>void;assistant:Record<string,unknown>|null;setAssistant:(v:Record<string,unknown>|null)=>void;notify:(text:string,tone?:string)=>void;toasts:Toast[];dismiss:(id:number)=>void;can:(...roles:string[])=>boolean};
...
export function AppProvider({children}:{children:ReactNode}){
 ...
 const [dark,setDark]=useState(localStorage.getItem("nexus-dark")==="true"),[collapsed,setCollapsed]=useState(false),[palette,setPalette]=useState(false),[assistant,setAssistant]=useState<Record<string,unknown>|null>(null),[toasts,setToasts]=useState<Toast[]>([]);
 ...
 useEffect(()=>{document.documentElement.classList.toggle("dark",dark);localStorage.setItem("nexus-dark",String(dark));},[dark]);
 ...
 function logout(){sessionStorage.removeItem("nexus-token");setUser(null);setWorkspace(null);setAssistant(null);setPalette(false);}
 return <Context.Provider value={{user,workspace,loading,error,refresh,login,logout,dark,toggleDark:()=>setDark(!dark),collapsed,setCollapsed,palette,setPalette,assistant,setAssistant,notify,toasts,dismiss:id=>setToasts(t=>t.filter(x=>x.id!==id)),can:(...roles)=>!!user&&(user.role==="Super Admin"||roles.includes(user.role))}}>{children}</Context.Provider>;
}
