...
export function AppProvider({children}:{children:ReactNode}){
 ...
 return <Context.Provider value={{selectedContext,setSelectedContext,user,workspace,loading,error,refresh,login,logout,collapsed,setCollapsed,palette,setPalette,assistant,setAssistant,notify,toasts,dismiss:id=>setToasts(t=>t.filter(x=>x.id!==id)),can:(...roles)=>!!user&&(["SuperAdmin","Registrar","Super Admin"].includes(user.role)||roles.includes(user.role)||(user.role==="RTE"&&roles.includes("Registrar")))}}>{children}</Context.Provider>;
}
