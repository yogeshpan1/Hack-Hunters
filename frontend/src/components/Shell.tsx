...
export default function Shell(){
 ...
 return <div className={`app-shell ${collapsed?"collapsed":""}`}>
  <aside className="sidebar">
   ...
   <nav aria-label="Main navigation">{items.map((item,index)=>{const Icon=item.icon;return <div key={item.path}>{(index===0||items[index-1].group!==item.group)&&<div className="nav-group">{item.group}</div>}<NavLink to={`/${item.path}`} className="nav-link" title={item.label}><Icon size={17}/><span>{item.label}</span>{item.path==="timetable"&&!!workspace?.conflicts.length&&<b className="nav-count">{workspace.conflicts.length}</b>}</NavLink></div>;})}</nav>
