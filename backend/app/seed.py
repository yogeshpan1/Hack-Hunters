...
from .models import User, Programme, Faculty, Cohort, Student, Room, Module, TimetableSession, ExamSession, Rule, ScheduleVersion, AuditLog
...
def seed(db):
    ...
    for i,(name,cap,kind,building) in enumerate(rooms,1):
        db.add(Room(id=i,name=name,capacity=cap,kind=kind,building=building,equipment=["Projector","AC","Whiteboard","Internet"]+(["Computers"] if kind=="Lab" else []),unavailable=[]))
    ...
    for c in db.scalars(select(Cohort)).all():
